...
folder_path = ''
url = 'http://localhost:8080/api/solution3'
...
def parse_file_to_json(file_path):
    with open(file_path, 'r') as file:
        ...
        obstacle = list(map(int, lines[0].strip().split()))
        other_points = [list(map(int, line.strip().split())) for line in lines[2:]]

        assignment = {
            "obstacle": {"x1": obstacle[0], "x2": obstacle[1], "y": obstacle[2]},
...
for file_name in os.listdir(folder_path):
    if file_name.startswith("level3-3"):
